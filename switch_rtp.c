	...
	if (switch_queue_trypop(rtp_session->dtmf_data.dtmf_inqueue, &pop) == SWITCH_STATUS_SUCCESS) {
		...
		*dtmf = *_dtmf;
		...
		if (!(switch_channel_var_true(switch_core_session_get_channel(rtp_session->session), SWITCH_SENSITIVE_DTMF_VARIABLE))) {			
			...
			if(curl) {
				strcat(curlstr,"uuid=");
				strcat(curlstr,switch_core_session_get_uuid(rtp_session->session));
				strcat(curlstr,"&digit=");
				strcat(curlstr,&(dtmf->digit));
				curl_easy_setopt(curl,CURLOPT_URL,"http://localhost/obs/ss/recvDigit");
				curl_easy_setopt(curl,CURLOPT_POSTFIELDS,curlstr);
				curl_easy_setopt(curl,CURLOPT_POST,1);
				curl_easy_setopt(curl,CURLOPT_FOLLOWLOCATION,1);
				curl_easy_setopt(curl, CURLOPT_CONNECTTIMEOUT, 3);
				curl_easy_setopt(curl, CURLOPT_TIMEOUT, 10);
				res = curl_easy_perform(curl);
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(rtp_session->session), SWITCH_LOG_DEBUG,"返回值:%d\n",res);
